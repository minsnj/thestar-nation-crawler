...
def get_sheet():
    ...
    if not ws.get_all_values() or ws.cell(1, 1).value != "날짜":
        ws.insert_row(["날짜", "제목 (EN)", "제목 (KR)", "요약 (KR)", "링크", "수집시각"], index=1)
    return ws
...
def fetch_articles():
    ...
    result = app.extract(
        urls=[NATION_URL],
        prompt="Extract all Nation news articles. For each: title, summary, url, published_date.",
        schema={
            "type": "object",
            "properties": {
                "articles": {
...
def run():
    ...
    for article, tr in zip(new_articles, translations):
        rows.append([
            article.get("published_date", now.strftime("%Y-%m-%d")),
    ...
    print(f"✓ {len(rows)}개 기사 구글시트 저장 완료!")
